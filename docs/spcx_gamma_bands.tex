...
The natural question is what constraint corrects this. A fixed per-ticket cost --- the
obvious candidate --- turns out to be negligible on this book (Section~7): what actually
limits a desk pushing 65\% of ADV through the market is capacity and footprint, not ticket
fees. The faithful instrument is therefore a direct cap on the average number of hedges
per day; we re-optimize both families under it.
...
frequent triggering. In the ES study the fix for this was a \emph{fixed per-ticket cost}.
That fix does \textbf{not} work on SPCX, and it is worth stating why, since the intuition
is misleading. Re-optimizing the whole grid under a fixed charge per child order, the
optimum does not move off the tight band until $C_{\text{ticket}}$ reaches roughly
\$20{,}000 \emph{per order} --- two to three orders of magnitude above any plausible
exchange, clearing or operational charge. A realistic \$10--\$100 per order is under
$0.2\%$ of execution cost and changes nothing. The reason is scale: impact here runs to
tens of millions of dollars because hedge clips are a large fraction of ADV, so a fee
charged per \emph{order} rather than per \emph{share} is a rounding error. (This is exactly
where SPCX and ES diverge: on ES, whose total execution cost is a thousand times smaller,
the same \$10--\$100 per order is $4$--$20\%$ of cost and does shift the optimum.)

What the SPCX model genuinely lacks is not a per-ticket cost but a \textbf{floor on order
splitting}: a minimum clip size, or a temporary-impact term that decays rather than
rewarding unbounded subdivision (splitting a parent into $n$ children costs
$Q^{1.5}n^{-1/2}$, which falls without limit in $n$). Absent that, the hedge-frequency cap
of Section~5 is the honest instrument --- and, as argued there, the operationally correct
one for a book of this size.
...
\begin{enumerate}
\item \textbf{A floor on order splitting.} The boundary solution comes from impact that
  rewards unbounded subdivision. A minimum clip size or a decaying temporary-impact term
  would make the optimal band width interior without an externally imposed frequency cap.
  (A fixed per-ticket cost, the analogous fix on ES, does not achieve this here --- it is
  negligible against this book's impact costs; see Section~7.)
