...
model without limit. Unconstrained grid search consequently runs away toward the tightest
band available --- a boundary solution, not a strategy. Capping the average number of
hedges per day restores an interior, decision-relevant optimum. On ES the same effect can
be obtained with a fixed per-ticket cost (execution cost there is small enough that a
plausible per-order charge bites); on SPCX, whose execution cost is a thousand times
larger, a per-ticket charge is negligible and the cap is the faithful instrument --- what
constrains a book pushing a large fraction of ADV is capacity and footprint, not ticket
fees. The results sections quantify this for each book.
...
capped. They also differ in how the boundary solution is best corrected: on ES a realistic
fixed per-ticket cost ($\sim$\$100 per order is $\sim$20\% of execution cost) moves the
optimum to a wider band, whereas on SPCX the same charge is negligible and the missing
ingredient is instead a floor on order splitting (a minimum clip size, or decaying
temporary impact). The dominant shared caveat is that absent such a floor, both cost
models reward unbounded subdivision; SPCX's short 22-day sample is a further caveat,
thin for distinguishing effects of a few percent.
