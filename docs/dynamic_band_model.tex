...
on SPCX, where a hedge clip is a meaningful fraction of a five-minute execution window's
liquidity even though it is small against full-day ADV: there $\alpha = Y\sigma_{\text{daily}}$
...
floor on order splitting, and impact is concave in clip size ($Q^{1.5}n^{-1/2}$ for $n$
children), so splitting a hedge into more, smaller child orders is strictly cheaper in the
model without limit. When impact is large enough, unconstrained grid search runs away toward
the tightest band available --- a boundary solution, not a strategy --- and capping the
average number of hedges per day restores an interior optimum. Whether that device is needed
is book-specific. On ES a fixed per-ticket cost bites (execution cost there is small enough
that a plausible per-order charge moves the optimum). On the SPCX book the optimum is already
an interior, wide band, so neither a per-ticket cost nor a frequency cap is required. The
results sections quantify this for each book.
...
\textbf{Data.} 22 trading days (June 16 -- July 17, 2026), 12{,}092 simulated SPCX option
executions (JPM modeled at $1\%$ of option flow), 1{,}716 five-minute bars. The stock traded
\$123.12--\$222.80 (median \$155.27) with ATM implied volatility of 70--126\% and a 20-day ADV
of $\approx60.9$M shares (\$9.5B/day), from real Nasdaq daily bars. Cost model: measured
half-spread \$0.027/share, $\alpha = Y\sigma_{\text{daily}} = 0.0273$, execution-window
liquidity $L_k = \text{ADV}\times 5/390$, closing-cross impact haircut, $M_h = 1$.
...
\textbf{Benchmark} & \textbf{Exec.\ cost} & \textbf{Risk (RMS/bar)} & $L_{\text{std}}$ \\
\midrule
Zero Hedge & \$154{,}125     & \$226{,}538 & 1.015 \\
Full Hedge & \$10{,}528{,}317 & \$0         & 1.000 \\
\bottomrule
\end{tabular}
\caption{SPCX benchmark bounds over 22 days. The selected strategy turns over
$\approx114{,}000$ shares/day, under $0.2\%$ of ADV. Impact dominates spread
($\approx\$0.69$ vs.\ \$0.027 per share all-in): the binding cost on this book is footprint,
...
\textbf{Gamma signal.} Median $\lvert\Gamma\rvert$ is $488$ shares/\$, with a within-day
coefficient of variation of $0.46$ and a max/median ratio of only $1.6\times$. Across bars
gamma correlates with $\lvert\Delta\rvert$ at $\rho = 0.53$: a good part of the gamma signal
is already in delta, and the range over which the band can flex is narrow.
...
\midrule
Static & 200{,}000 & ---                & \$1.74M & \$78{,}303 & 0.5110 \\
Gamma  & 300{,}000 & $2\times10^{-4}$   & \$1.74M & \$78{,}303 & 0.5110 \\
\bottomrule
\end{tabular}
\caption{Best configuration per family (VWAP, 30min). Both cut the objective by
$\approx50\%$ against Zero Hedge and $\approx49\%$ against Full Hedge --- the first-order
result on this book. Best static and best gamma are \emph{identical} to four decimals: the
winning tilt ($c = 2\times10^{-4}$) leaves the band static in all but name.}
\end{table}

\begin{whybox}
\textbf{The optimum is a wide, interior band.} Because the impact proxy rewards splitting a
hedge into ever-smaller children, one must check the optimum is not sitting on the tight edge
of the grid. It is not: the static ladder is \emph{U-shaped} with an interior minimum
($H_0 = 5$k: $0.552$; $25$k: $0.569$; $100$k: $0.565$; $\mathbf{200}$\textbf{k}:
$\mathbf{0.511}$; $300$k: $0.712$). The best band is \emph{wide} and fires only $\approx0.4$
hedges/day; tighter bands are strictly worse because impact is not negligible. The grid
optimum is well-behaved, needing no per-ticket cost or hedge-frequency cap to make it interior.
\end{whybox}

\textbf{Width-matched comparison.} Width-for-width the gamma band remains mildly
risk-efficient across the widths that bind, but the edge does not carry to the optimum.
...
\bottomrule
\end{tabular}
\caption{Width-matched gamma vs.\ static at $c = 10^{-3}$, TWAP 60min. The edge is $+0.4\%$
to $+2.4\%$ across binding widths and collapses at the widest setting, where the band rarely
binds and the tilt merely over-tightens.}
\end{table}

\textbf{The operational regime.} A hedge-frequency cap does not bind: the grid optimum
already fires $\approx0.4$ hedges/day --- below even a 5/day cap --- so capping changes
nothing and static wins or ties at every level.
...
because the wide-band optimum fires far below the tightest cap.}
\end{table}

Because the optimum hedges rarely, there is no scarce hedge budget whose allocation gamma
could improve --- allocating hedges to high-convexity moments, which
Equation~\eqref{eq:gammaband} optimizes, only helps when hedges are scarce. Across
$\lambda_{\text{risk}} \in [0.25, 4]$ the gamma band likewise only ties or leads the static
band by $\le +0.4\%$.

\textbf{Band-edge targeting.} Mildly positive for moderate widths of either shape, negative
only for the widest bands.
...
Carrying residual delta to a cheap auction close is most of what edge targeting buys, and on
SPCX the closing cross already delivers that to every strategy, so the effect is small and
unsystematic --- a minor tuning knob, not a driver.

\textbf{Comparison with ES.} The gamma-conditioned band is risk-efficient at a matched width
on both books ($2.5$--$4.0\%$ on ES, up to $\approx2.4\%$ on SPCX), but its operational value
differs. On ES a realistic per-ticket cost bites and edge targeting stacks with the band. On
SPCX, hedging is cheap relative to the book, the optimum is a wide, rarely-firing band, and
neither gamma conditioning nor a hedge-frequency cap changes the ranking. The shared caveat is
the frictionless proxy's lack of a floor on order splitting; SPCX's short 22-day sample is a
further one.
